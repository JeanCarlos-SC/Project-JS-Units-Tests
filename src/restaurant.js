...
// PASSO 3: Crie uma função, separada da função `createeMnu()`, que, ao receber uma string como parâmetro, 
...
const createMenu = (obj) => {
  ...
  fetchMenu: () => obj,
  consumption: itens,
  order: (request) => itens.push(request),
  pay: () => (sumFoods() + sumDrinks()),
  clearOrder: () => {
      while (itens.length) {
      itens.pop();
    }
...
sumFoods = () => {
  ...
  const foods = Object.keys(meuRestaurante.fetchMenu().food);
  const consum = meuRestaurante.consumption;
  let sum = 0;
  for (let index = 0; index < consum.length; index += 1) {
    if (foods.includes(consum[index])) {
      let order = meuRestaurante.fetchMenu().food;
      sum += order[consum[index]];
    }
  }
  const ten = (sum / 100) * 10;
  return parseFloat((sum + ten).toPrecision(3));
};
...
sumDrinks = () => {
  ...
  const drinks = Object.keys(meuRestaurante.fetchMenu().drink);
  const consum = meuRestaurante.consumption;
  let sum = 0;
  for (let index = 0; index < consum.length; index += 1) {
    if (drinks.includes(consum[index])) {
      let order = meuRestaurante.fetchMenu().drink;
      sum += order[consum[index]];
    }
  }
  const ten = (sum / 100) * 10;
...
module.exports = { createMenu, sumFoods, sumDrinks };
