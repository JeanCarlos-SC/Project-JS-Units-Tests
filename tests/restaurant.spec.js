const {createMenu, sumFoods, sumDrinks } = require('../src/restaurant');
...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    objetoRetornado = createMenu({ food: {coxinha: 3.9, sopa: 9.9, sashimi: 15.9}, drink: {agua: 3.9, cerveja: 6.9} });
    expect(objetoRetornado.pay()).toBeCloseTo(45.58);
  
    // Agora faça o PASSO 4 no arquivo `src/restaurant.js`.
  });
});

describe('Testando o comportamento da função em diferentes casos', () => {
  it("Testando a função clearOrder(), que deve limpar o array 'consumption'", () => {
    menuRestaurante = createMenu({ food: {coxinha: 3.9, sopa: 9.9, sashimi: 15.9}, drink: {agua: 3.9, cerveja: 6.9} });
    menuRestaurante.clearOrder();
    expect(menuRestaurante.consumption).toEqual([]);
  });

  it("Quando o consumption recebe a string 'coxinha' o valor final esperado é 4.29'", () => {
    menuRestaurante.order('coxinha');
    expect(menuRestaurante.pay()).toBeCloseTo(4.29);
  });

  it("Quando o consumption recebe as strings 'coxinha' e 'agua' o valor final esperado é 8.58'", () => {
    menuRestaurante.order('agua');
    expect(menuRestaurante.pay()).toBeCloseTo(8.58);
  });

  it("Quando o consumption não recebe nada deve retornar 0'", () => {
    menuRestaurante.clearOrder();
    expect(menuRestaurante.pay()).toBeCloseTo(0);
  });
})
